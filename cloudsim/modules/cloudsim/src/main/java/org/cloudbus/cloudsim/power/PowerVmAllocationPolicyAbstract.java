...
public abstract class PowerVmAllocationPolicyAbstract extends VmAllocationPolicy {
	...
	//存一个分配表
	private Map<String, Host> vmTableCopy = new HashMap<>();
	//存performance degradation
	private double performanceDegradation = 0;
	...
	public boolean allocateHostForVm(Vm vm, Host host) {
		...
		if (host.vmCreate(vm)) { // if vm has been succesfully created in the host
			...
			//try copy
			getVmTableCopy().putIfAbsent(vm.getUid(), host);
			//try calculate
			//setPerformanceDegradation(host.getPerformance());
			Log.formatLine(
...
//	/*
//	因为要保存vmTable，故在placement做完之后进行保存
//	 */
//	public void setVmCopy(){
//		getVmTableCopy().putAll(getVmTable());
//	}

	public Map<String, Host> getVmTableCopy(){
		return vmTableCopy;
	}
	...
	public double getPerformance(){
		return this.performanceDegradation;
	}
}
