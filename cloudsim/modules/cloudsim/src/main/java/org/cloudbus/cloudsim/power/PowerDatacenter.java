...
public class PowerDatacenter extends Datacenter {
	...
	private double totalNormalizedEnergy = 0;

	public double getTotalNormalizedEnergy(){
		return totalNormalizedEnergy;
	}

	private ArrayList<Double> hostUtilizationList = new ArrayList<>();

	private void initialHostUtilizationList(){
		for(int i = 0; i < getHostList().size(); i ++){
			getHostUtilizationList().add(0.0);
		}
	}
	public ArrayList<Double> getHostUtilizationList(){
		return this.hostUtilizationList;
	}

	private int utilizationCount = 0;
	public int getUtilizationCount(){
		return this.utilizationCount;
	}

	public void addUtilizationCount(){
		++ utilizationCount;
	}
	/**
	...
	protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
		...
		if (timeDiff > 0) {
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				//added by pp
				totalNormalizedEnergy += timeFrameHostEnergy / (host.getMaxPower() * timeDiff);
