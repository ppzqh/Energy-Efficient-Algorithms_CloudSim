...
public class Helper {
	...
	public static double getPowerByHistory(List<PowerHost> hosts){
		...
		for (PowerHost host : hosts) {
			...
			for (int i = 1; i < stateHistory.size(); i ++) {
				double utilFrom = stateHistory.get(i-1).getAllocatedMips() / host.getTotalMips();
				double utilTo = stateHistory.get(i).getAllocatedMips() / host.getTotalMips();// - stateHistory.get(i-1).getAllocatedMips();
				double timeDiff = stateHistory.get(i).getTime() - stateHistory.get(i-1).getTime();

//				double powerFrom = host.getPower(utilFrom);
//				double powerTo = host.getPower(utilTo);
//				totalEnergy += (powerFrom + (powerTo - powerFrom) / 2) * timeDiff;
				totalEnergy += host.getEnergyLinearInterpolation(utilFrom, utilTo, timeDiff);
			}
		}
		return totalEnergy / (3600 * 1000);


	}


	public static void saveResult(HashMap<String,String> simulationResult, String fileName) throws IOException{
		String[] metric_list = {"Energy consumption", "Average SLA violation", "Overall SLA violation",
				"num of VM migrations", "num of active host", "SLA"};
		try{
			File file = new File(fileName);
			FileWriter out = new FileWriter(file, true);
			for(String metric:metric_list){
				out.write(simulationResult.get(metric)+" ");
			}
	...
	public static List<Cloudlet> createCloudlet(int userId, int cloudlets){
		...
		//cloudlet parameters
		long length = Constants.CLOUDLET_LENGTH;
		long fileSize = 300;
		long outputSize = 300;
		int pesNumber = 1;
	...
	public static List<Cloudlet> createCloudlet(int userId, int cloudlets, int idShift){
		...
		for(int i=0;i<cloudlets;i++){
			cloudlet[i] = new Cloudlet(i+idShift, length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
			// setting the owner of these Cloudlets
			cloudlet[i].setUserId(userId);
			list.add(cloudlet[i]);
		}
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			//记得换个写法
			vms.add(new PowerVmLearningAutomata(
					i,
					brokerId,
	 ...
	 * @return the list< vm>
	 */
	public static List<Vm> createVmList(int brokerId, int vmsNumber, int idShift) {
		List<Vm> vms = new ArrayList<Vm>();
		for (int i = 0; i < vmsNumber; i++) {
			//int vmType = i / (int) Math.ceil((double) vmsNumber / Constants.VM_TYPES);
			int vmType = i % 4;
...
//					new CloudletSchedulerTimeShared(),
	...
	public static PowerHost createHost(int id){
		int hostType = id % Constants.HOST_TYPES;
...
//			broker = new DatacenterBrokerUncertain("Broker");
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
...
//			broker = new DatacenterBrokerUncertain("Broker");
	 ...
	 * @return the datacenter broker
	 */
	public static DatacenterBroker createBrokerUncertain() {
		DatacenterBroker broker = null;
		try {
			broker = new DatacenterBrokerUncertain("Broker");
		} catch (Exception e) {
	...
	public static PowerDatacenter createDatacenter(
		...
		try {
			datacenter = (PowerDatacenter) datacenterClass.getConstructor(
					String.class,
	...
	protected static int getActiveHostNum(List<Host> hosts){
		int activeNum = 0;
		int historyNum = 0;
		for (Host _host : hosts) {
			HostDynamicWorkload host = (HostDynamicWorkload) _host;
			if (host.getStateHistory().size() != historyNum) {
				historyNum = host.getStateHistory().size();
			}
			for (HostStateHistoryEntry entry : host.getStateHistory()) {
				if(entry.isActive()) ++ activeNum;
			}
		}
		if(historyNum == 0) return activeNum;
		return activeNum / historyNum;
	}
	...
	//pp修改了返回值
	public static HashMap<String,String> printResults(
		...
		//active host num
		int activeHostNum = getActiveHostNum(hosts);
...
//			returnResult.put("Experiment name", experimentName);
//			returnResult.put("num of Hosts", Integer.toString(numberOfHosts));
//			returnResult.put("num of VMs", Integer.toString(numberOfVms));
			returnResult.put("Energy consumption", Double.toString(energy));
			returnResult.put("num of VM migrations", Integer.toString(numberOfMigrations));
			returnResult.put("num of active host", Integer.toString(activeHostNum));

			returnResult.put("Overall SLA violation", Double.toString(slaOverall * 100));
			returnResult.put("Average SLA violation", Double.toString(slaAverage * 100));
			returnResult.put("SLA", Double.toString(sla * 100));
			...
			//active host
			Log.printLine(String.format("Number of active host: %d", activeHostNum));
			//self-calculated energy
...
//					System.out.println("allocated:"+previousAllocated+" requested: "+previousRequested);
					if (previousAllocated < previousRequested) {
						//SLA violation
						//System.out.println("Temp SLA violation: "+ (previousRequested - previousAllocated) / previousRequested);
...
//	}
	...
	/*
	save data of energy consumption
	 */
	public static void saveDataEnergy(String data, String fileName){
		try{
			File file = new File(fileName);
			FileWriter out = new FileWriter(file, true);
			out.write(data);
	...
	save data of energy consumption
	 */
	public static void saveDataEnergy(Double data, Double data1, String fileName){
		try{
			File file = new File(fileName);
			FileWriter out = new FileWriter(file, true);
			out.write(Double.toString(data));
			out.write("_"+Double.toString(data1));
			out.write(System.getProperty("line.separator"));
			out.close();
		} catch(IOException i){
			System.out.println("save result failed");
	...
	public static List<Cloudlet> createCloudletStaticUtilization(
			Map< Integer, Double> cpuUtilizationForVM,
			Map< Integer, Double> memoryUtilizationForVM) {
		int vmNum = cpuUtilizationForVM.size();
		List<Cloudlet> cloudletList = new ArrayList<>();
		for(int cloudletId = 0; cloudletId < vmNum; cloudletId ++){
			...
			try {
				cloudlet = new Cloudlet(
						...
						300,
						300,
						new UtilizationModelStatic(cpuUtilizationForVM.get(cloudletId)),
						new UtilizationModelStatic(memoryUtilizationForVM.get(cloudletId)),
